...
int space_start() {
    ...
    /* SpaceObject debris;s
    debris.position = (Vector3){0, 100000, orbital_radius};
    debris.velocity = (Vector3){sqrt(G * M / orbital_radius), -3000, 0};
    debris.mass = 10;  // 20 kg
    */

    for (int i = 0; i < 2000; i++) {
        //SpaceObject temp_space_object = {(Vector3){0, 0, 1000000}, (Vector3){7000, 0, 0}, 0.5};
        ...
        /*int orbit_radius_x = 1+rand() % 1000;
        int orbit_radius_y = rand() % 1000;
        int orbit_radius_z = EARTH_RADIUS + 700000 + rand() % 500000;

        SpaceObject temp_space_object = {(Vector3){orbit_radius_x, orbit_radius_y, orbit_radius_z}, (Vector3){sqrt(G * M / (orbit_radius_z)), sqrt(G * M / (orbit_radius_x)), sqrt(G * M / (orbit_radius_y))}, 0.5};
        space_objects[i] = temp_space_object;  */   
    }
    ...
    for (int step = 1; step < steps; step++) {
        ...
        */
        ...
        for (int i = 0; i < 2000; i++) {
            ...
            int xz_renderer_x = (int)((space_objects[i].position.x / (float)20000000) * GLOBAL_SCREEN_WIDTH/2) + GLOBAL_SCREEN_WIDTH / 4;
            int xz_renderer_y = (int)((space_objects[i].position.z / (float)20000000) * GLOBAL_SCREEN_HEIGHT) + GLOBAL_SCREEN_HEIGHT / 2;
            renderer_set_pixel(xz_renderer_x, xz_renderer_y);

            int xy_renderer_x = 1 + (int)((space_objects[i].position.x / (float)20000000) * GLOBAL_SCREEN_WIDTH/2) + 3*GLOBAL_SCREEN_WIDTH / 4;
            int xy_renderer_y = (int)((space_objects[i].position.y / (float)20000000) * GLOBAL_SCREEN_HEIGHT) + GLOBAL_SCREEN_HEIGHT / 2;
            renderer_set_pixel(xy_renderer_x, xy_renderer_y);
            /*printf("Step %d: Position = (%f, %f, %f) \tVelocity = (%f, %f, %f)\n", step,
